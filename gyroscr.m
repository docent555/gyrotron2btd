...
for step=1:Nt-1
    ...
    d   = @(cu, cu_p, WL, WR, DPART) DAR(cu, cu_p, WL, WR, DPART, Nz, C1, C2, coeff_4_d_3_m_SQRDT, coeff_1i_m_SQRDZ);
    ...
    WL(IDX(step)) = w( 1, cu(1),  cu(2),    cu(1),  cu(2),    WL_PART);    
    WR(IDX(step)) = w(-1, cu(Nz), cu(Nzm1), cu(Nz), cu(Nzm1), WR_PART);

    % WL(IDX(step)) = coeff_dz_m_coeff_1i_d_6*(4.0D0 * cu(1) + 2.0D0 * cu(2)) + WL_PART;
    ...
    D = d(cu(2:Nzm1), cu(2:Nzm1), WL(IDX(step)), WR(IDX(step)), D_PART);    
    ...
    while 1
        ...
        WL(IDX(step)) = w( 1, cu(1),  cu(2),    cu_p(1),  cu_p(2),    WL_PART);    
        WR(IDX(step)) = w(-1, cu(Nz), cu(Nzm1), cu_p(Nz), cu_p(Nzm1), WR_PART);
        ...
        D = d(cu(2:Nzm1), cu_p(2:Nzm1), WL(IDX(step)), WR(IDX(step)), D_PART);
    ...
    Sigma0(IDX(step))    = sgm(field(1),    F0(IDX(step - 1)),    cu(1),     J0(IDX(step - 1)),    Sigma0(IDX(step - 1)));    
    Sigma1(IDX(step))    = sgm(field(2),    F1(IDX(step - 1)),    cu(2),     J1(IDX(step - 1)),    Sigma1(IDX(step - 1)));    
    SigmaNz(IDX(step))   = sgm(field(Nz),   FNz(IDX(step - 1)),   cu(Nz),    JNz(IDX(step - 1)),   SigmaNz(IDX(step - 1)));
    SigmaNzm1(IDX(step)) = sgm(field(Nzm1), FNzm1(IDX(step - 1)), cu(Nzm1),  JNzm1(IDX(step - 1)), SigmaNzm1(IDX(step - 1)));    
